...
pub struct LipschitzEstimator<'a, F, T>
where
...
impl<'a, F, T> LipschitzEstimator<'a, F, T>
where
    F: Fn(&[T], &mut [T]) -> Result<(), SolverError>,
    T: OptFloat,
{
    ...
    pub fn new(
        ...
        LipschitzEstimator {
            ...
            function: f_,
            epsilon_lip: T::from(DEFAULT_EPSILON).unwrap(),
            delta_lip: T::from(DEFAULT_DELTA).unwrap(),
        }
    }
